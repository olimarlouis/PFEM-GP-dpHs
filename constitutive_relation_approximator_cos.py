...
class PFEM_GP_PHS_WAVE:
    def __init__(self,dx,dx_theta,Nmq,Nmp,Nlq,Nlp):
        self.best_h = None
        ...
        self.init_mq(np.zeros(Nmq))
        self.init_mp(np.zeros(Nmp))
        
        self.init_lq(np.zeros(Nlq))
        self.init_lp(np.zeros(Nlp))


        self.mq_data = np.zeros(self.Nmq)
        self.mp_data = np.zeros(self.Nmp)

        self.lq_data = np.zeros(self.Nlq)
        self.lp_data = np.zeros(self.Nlp)
    ...
    def create_polynome(self,v):
        expr = str(v[0])
        for i in range(1,len(v)):
            coeff = v[i]
            if i%2 == 0:
                if coeff<0:
                    expr += '-'+str(-coeff)+'*cos(2*pi*X*'+str(i//2)+')'
                else:
                    expr += '+'+str(coeff)+'*cos(2*pi*X*'+str(i//2)+')'
            else:
                if coeff<0:
                    expr += '-'+str(-coeff)+'*sin(2*pi*X*'+str((i+1)//2)+')'
                else:
                    expr += '+'+str(coeff)+'*sin(2*pi*X*'+str((i+1)//2)+')'
    ...
    def set_mq(self, v):
        self.md.del_macro('mq')
        self.md.add_macro('mq',self.create_polynome(v))
        self.mq_data = v
        self.Nmq = len(v)
        for ii in range(self.Nmq):
            vv = np.zeros(self.Nmq)
            vv[ii] = 1
            self.md.del_macro('dmq'+str(ii))
            self.md.add_macro('dmq'+str(ii),self.create_polynome(vv))



    def set_mp(self, v):
        self.md.del_macro('mp')
        self.md.add_macro('mp',self.create_polynome(v))
        self.mp_data = v
        self.Nmp = len(v)
        for ii in range(self.Nmp):
            vv = np.zeros(self.Nmp)
            vv[ii] = 1
            self.md.del_macro('dmp'+str(ii))
            self.md.add_macro('dmp'+str(ii),self.create_polynome(vv))


    def set_lq(self, v):
        self.md.del_macro('lq')
        self.md.add_macro('lq',self.create_polynome(v))
        self.lq_data = v
        self.Nlq = len(v)
        for ii in range(self.Nlq):
            vv = np.zeros(self.Nlq)
            vv[ii] = 1
            self.md.del_macro('dlq'+str(ii))
            self.md.add_macro('dlq'+str(ii),self.create_polynome(vv))
        

    def set_lp(self, v):
        self.md.del_macro('lp')
        self.md.add_macro('lp',self.create_polynome(v))
        self.lp_data = v
        self.Nlp = len(v)
        for ii in range(self.Nlp):
            vv = np.zeros(self.Nlp)
            vv[ii] = 1
            self.md.del_macro('dlp'+str(ii))
            self.md.add_macro('dlp'+str(ii),self.create_polynome(vv))
    ...
    def update_all_hyperparameter(self,h):
        ...
        self.set_mq(h[:Nmq])
        self.set_mp(h[Nmq:Nmq+Nmp])
        self.set_lq(h[Nmq+Nmp:Nmq+Nmp+Nlq])
        self.set_lp(h[Nmq+Nmp+Nlq:Nmq+Nmp+Nlq+Nlp])

        # lqq = [0 for i in range(nq)]
        # lqq[1:nq-1] = h[0:nq-2]
        # self.set_lq(lqq)
        self.set_sigma_f(h[Nmq+Nmp+Nlq+Nlp])
        self.set_sigma_q(h[Nmq+Nmp+Nlq+Nlp+1])
        self.set_sigma_p(h[Nmq+Nmp+Nlq+Nlp+1])
